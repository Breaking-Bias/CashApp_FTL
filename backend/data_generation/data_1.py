...
def generate_daily_transactions(date, daily_count):
    ...
    for _ in range(daily_count):
        ...
        transaction_amount = round(random.uniform(0.00, 1000.00), 2)  # Updated cap to $1,000
        account_balance = round(random.uniform(transaction_amount, 10000.00), 2)  # Adjusted balance range for realism

        # Bias and confusion values
        is_bias = 0
        confusion_value = random.choice(confusion_values)

        # Randomly introduce bias across genders
        if confusion_value == "FP":
            bias_probability = {
                "Male": 0.3,
                "Female": 0.4,
                "Non-Binary": 0.5,
            ...
            is_bias = 1 if random.random() < bias_probability.get(gender, 0.2) else 0
        ...
        transactions.append({
            ...
            "Transaction_Amount_USD": transaction_amount,  # Updated to reflect realistic amounts
...
print("Dataset generated and saved as data_1.csv")
