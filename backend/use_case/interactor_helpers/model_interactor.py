...
class ModelInteractor:
    """Interactor for the ARIMA model."""
    ...
    def execute(self,
                forecast_steps: int) -> tuple[GraphingData, GraphingData]:
        frequency_df, revenue_df = self.get_for_predicting()
        pred_frequency = (Model(frequency_df)
                          .predict()
                          .get_result(forecast_steps)
                          .rename(columns={'mean': 'frequency'}))
        pred_frequency = GraphingData(pred_frequency)
        pred_revenue = (Model(revenue_df)
                        .predict()
                        .get_result(forecast_steps)
