...
class DataFactory:
    ...
    def __init__(self, dataset: pd.DataFrame, filter_gender: str, filter_race: str, forecast_steps: int):
        ...
        self.frequency_past_data_biased = DataFormatter(biased_dataset.copy()).get_frequency_data()
        self.revenue_past_data_biased = DataFormatter(biased_dataset.copy()).get_revenue_data()
        self.frequency_past_data_unbiased = DataFormatter(unbiased_dataset.copy()).get_frequency_data()
        self.revenue_past_data_unbiased = DataFormatter(unbiased_dataset.copy()).get_revenue_data()

    def filter_biased(self) -> pd.DataFrame:
        filtered_biased_dataset = (FilterInteractor(self.dataset.copy())
    ...
    def filter_unbiased(self) -> pd.DataFrame:
        filtered_unbiased_dataset = (FilterInteractor(self.dataset.copy())
                                .filter_by(self.filter_gender, self.filter_race)
                                .unbias()
                                .filter_invalid_transactions())
